#!/usr/bin/env python3
"""Validate every workshops/<slug>/<lang>/README.md against the workshop format.
...
def load_workshops(root=ROOT):
    """Yield (path, frontmatter, body) for every workshops/*/*/README.md. Unparseable files yield frontmatter=None."""
    for path in sorted((root / "workshops").glob("*/*/README.md")):
        try:
...
def check_file(path, fm, body, slugs, root):
    ...
    errors, warnings = [], []
    if fm is None:
        return [body], []  # body holds the parse error
    ...
    if fm.get("language") != lang:
        errors.append(f"frontmatter: language '{fm.get('language')}' must equal folder name '{lang}'")
    if fm.get("level") not in LEVELS:
        errors.append(f"frontmatter: level must be one of {sorted(LEVELS)}")
    if "last_updated" in fm and not is_date(fm["last_updated"]):
        errors.append("frontmatter: last_updated must be a plain date (YYYY-MM-DD), no time")
    if "archived" in fm and not isinstance(fm["archived"], bool):
        errors.append("frontmatter: archived must be true or false (unquoted)")
    if "order" in fm and not (isinstance(fm["order"], int) and not isinstance(fm["order"], bool)):
        errors.append("frontmatter: order must be an integer")
        ...
        for person in fm.get(key) if isinstance(fm.get(key), list) else []:
            if isinstance(person, dict):
                if not isinstance(person.get("name"), str):
                    errors.append(f"frontmatter: entries in '{key}' need a 'name'")
                if "url" in person and not str(person["url"]).startswith("http"):
                    errors.append(f"frontmatter: '{key}' url must start with http")
            elif not isinstance(person, str):
                errors.append(f"frontmatter: entries in '{key}' must be a name or {{name, url}}")

    source = fm.get("translated_from")
    siblings = {p.name for p in path.parent.parent.iterdir() if (p / "README.md").exists()}
    if source and not (isinstance(fm.get("translators"), list) and fm["translators"]):
        errors.append("frontmatter: translations need at least one entry in 'translators'")
    if source:
        m = SOURCE_RE.match(str(source))
        if not m:
            errors.append(f"frontmatter: translated_from '{source}' must look like en@2026-09-03 (source language @ its last_updated)")
        elif m.group(1) == lang or m.group(1) not in siblings:
            errors.append(f"frontmatter: translated_from points to '{m.group(1)}', which is not another language of this workshop")
        else:
            try:
                src_date = date.fromisoformat(m.group(2))
            except ValueError:
                errors.append(f"frontmatter: translated_from date '{m.group(2)}' is not a real date")
            else:
                src_fm = split_frontmatter((path.parent.parent / m.group(1) / "README.md").read_text(encoding="utf-8"))[0]
                src_updated = src_fm.get("last_updated") if isinstance(src_fm, dict) else None
                if src_date > date.today():
                    errors.append("frontmatter: translated_from date is in the future")
                elif is_date(src_updated) and src_date > src_updated:
                    errors.append(f"frontmatter: translated_from date is later than {m.group(1)}'s last_updated ({src_updated})")

    for pre in fm.get("prerequisites") if isinstance(fm.get("prerequisites"), list) else []:
        if pre not in slugs:
            errors.append(f"frontmatter: prerequisite '{pre}' is not a workshop slug")

    lines = body.splitlines()
    h1 = [l for l in lines if l.startswith("# ")]
    if not h1:
        errors.append("body: missing '# <Title>' heading")
    elif isinstance(fm.get("title"), str) and h1[0][2:].strip() != fm["title"].strip():
        errors.append(f"body: heading '{h1[0][2:].strip()}' must equal frontmatter title '{fm['title']}'")
    ...
    if lang == "en":
        ...
        if missing:
            errors.append(f"body: required headings missing or out of order: {', '.join(missing)}")
        for h in OPTIONAL_HEADINGS:
            if h not in h2:
                warnings.append(f"body: optional section '{h}' is missing")
    else:
        # ponytail: non-English headings can't be matched by text; go by section count
        if len(h2) < len(REQUIRED_HEADINGS):
            errors.append(f"body: expected at least {len(REQUIRED_HEADINGS)} '## ' sections (overview, prerequisites, workshop, resources), found {len(h2)}")
        elif len(h2) < len(HEADINGS):
            warnings.append(f"body: {len(h2)} '## ' sections; the template has {len(HEADINGS)} (learning objectives, exercises, next steps are optional)")
    ...
    for target in LINK_RE.findall(body):
        ...
        if rel and not (path.parent / rel).exists():
            errors.append(f"body: link '{target}' does not resolve")
...
def validate(root=ROOT, warnings=None):
    ...
    for slug in sorted(slugs):
        ...
        if files and not originals:
            problems.append((files[0][0], "every workshop needs one original file without translated_from"))
...
if __name__ == "__main__":
    ...
    print(f"{len(problems)} error(s), {len(warnings)} warning(s) in {n} file(s)")
